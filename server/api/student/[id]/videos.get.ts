...
export default defineEventHandler(async (event) => {
  ...
  const [potentialVideos] = await pool.query(potentialVideosQuery, [student_id])
  ...
  const missingVideos = (potentialVideos as svRow[]).filter(
    (video: svRow) => !existingVideoIds.includes(video.video_id),
  )

  if (missingVideos.length > 0) {
    const insertValues = missingVideos.map((video: svRow) => [
      student_id,
      video.video_id,
      0,
      0,
